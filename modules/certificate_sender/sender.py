...
class Sender:
    ...
    def send(self, folder_name, data_template, confirm_mode):
        ...
        subject, content = self.parse_message(
            os.path.join(data_path, data_template['message']))

        # Load recipients data
        recipients = pd.read_csv(os.path.join(
            data_path, data_template['recipient']))
        ...
        for i, row in recipients.iterrows():
            ...
            # Cuma untuk git
            # content1 = content.replace('[NAME]', row['Name'].strip())
            # content2 = content1.replace('[TOKEN]', row['Token'].strip())
            # certificate_path = os.path.join(
            #     certificate_output_path, row['Name'].upper() + '.pdf')
            # self.api.send_certificate(
            #     row['Email'], subject, content2, certificate_path)
            # batas bawah cuma untuk git
            certificate_path = os.path.join(
                certificate_output_path, row['Name'].upper() + '.pdf')
            self.api.send_certificate(row['Email'], subject, content.replace(
                '[NAME]', row['Name'].strip()), certificate_path)
    ...
    def generate(self, folder_name, data_template):
        ...
        for i, row in tqdm(recipients.iterrows(), total=len(recipients)):
            ...
            certificate_path = generator.generate(
                row['Name'], certificate_code)
            certificate_url = self.api.upload_certificate_to_drive(
                certificate_path, data_template['folder_id'])
            certificate_info = get_certificate_info(name=row['Name'],
                                                    code=certificate_code,
                                                    url=certificate_url,
                                                    valid_until='Forever')
            result_df.append(certificate_info)
        pd.DataFrame(result_df).to_csv(os.path.join(
            certificate_output_path, 'result.csv'))

    def get_data_templates(self):
        template_folders = self.get_data_templates_folders()
        data_templates = {}
        for folder_name in template_folders:
            data_templates[folder_name] = self.get_data_template_components(
                folder_name)
    ...
    def get_data_template_components(self, folder_name):
        ...
        recipients = list(filter(lambda x: os.path.splitext(x)[
                          1] == '.csv', component_filenames))
        message = list(filter(lambda x: os.path.splitext(x)
                       [1] == '.txt', component_filenames))
        code = list(filter(lambda x: os.path.splitext(x)
                    [1] == '.code', component_filenames))
        certificate = list(filter(lambda x: os.path.splitext(x)[
                           1] == '.pdf', component_filenames))
        if len(certificate) == 0:
            certificate = list(filter(lambda x: os.path.splitext(x)[
                               1] == '.png', component_filenames))
    ...
    def validate_components(self, folder_name, components):
        for component_name, value in components.items():
            ...
            if len(value) == 0:
                raise errors.RequiredComponentNotExist(
